...
return function(Http)
	...
	local function buildRules()
		local rules = {
			...
			"- Lean context: nothing about the current game, persisted memory, or skill guides is included in this prompt by default. Pull what you need as you need it using the tools below.",
			"- BEFORE substantive work (writing code, building UI, scanning remotes, anything beyond chitchat): call list_skills() to see available guides and call recall() to check for persisted facts about this game. Use get_skill(name) for any guide whose description matches the task. SKIP all this for greetings, simple factual questions, or single-tool lookups — no recall/list needed for 'hi', 'how many players', 'what's my walkspeed'.",
			"- For game-state questions (what's in workspace, how many players, what scripts exist), use tree('game', 2) or find_class()/find_name() — do NOT assume any structure.",
			"- NEVER tell the user a function/capability is unavailable without verifying first. If unsure whether function X exists (firetouchinterest, hookmetamethod, fireclickdetector, WebSocket, Drawing, crypt.*, sethiddenproperty, etc.), call run_once('print(type(X))') or unc_test for the full report BEFORE answering. Default assumption: modern executors support these — only claim 'not supported' AFTER a check returns nil/false. Wrongly refusing a capability is worse than trying and failing.",
