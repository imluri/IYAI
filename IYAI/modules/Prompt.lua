...
return function(Http)
	...
	local function buildRules()
		local rules = {
			...
			"- Skill Guides: BEFORE doing actual work (writing code, building a UI, scanning remotes, anything beyond chitchat), scan the Skill Guides index in this prompt and call get_skill(name) for any guide whose description matches the task. SKIP this for greetings, small talk, simple factual questions, or single-tool lookups — no guide call needed for 'hi', 'how many players', 'what's my walkspeed'. Match required for: GUI/window/HUD requests, ESP/Drawing, remote scanning/hooking, metatable tricks, garbage-collection scans, interaction triggers (click/touch/prompt firing).",
